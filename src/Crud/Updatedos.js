...
export default class Update extends Component {
   ...
   addPost(e){
    e.preventDefault();
    if(this.state._id) {
      fetch(`http://localhost:4000/api/postdos/${this.state._id}`, {
        method: 'PUT',
        ...
        .then(res => res.json())
        .then(data => {
          this.setState({name: '', title: '', image: '', description: '', price: '', amount: '',_id: '' });
          this.fetchPosts();
        });

    }else{
           // fetch('https://stelenapp.herokuapp.com/api/post/add',{       
            fetch('http://localhost:4000/api/postdos/add',{
              method: 'POST',
              body: JSON.stringify(this.state),
   ...
   fetchPosts() {
    ...
    fetch('http://localhost:4000/api/postdos')
      .then(res => res.json())
      .then(data => {
        console.log(data);
  ...
  deletePost(_id){ 
       // fetch(`https://stelenapp.herokuapp.com/api/post/delete/${_id}`, {
          fetch(`http://localhost:4000/api/postdos/delete/${_id}`, {
          method: 'DELETE',
          headers: {
            'Accept': 'application/json',
            'Content-Type': 'application/json'
    ...
    editPost(_id) {
      fetch(`http://localhost:4000/api/postdos/${_id}`)
        .then(res => res.json())
        .then(data => {
          console.log(data);
          this.setState({

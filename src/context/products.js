...
export default function ProductProvider({ children }) {
   ...
   const [productdos, setProductdos] = React.useState([]);
 ...
 //  const initialUrlDos = `https://stelenapp.herokuapp.com`;
   ...
   const initialUrltres = `http://localhost:4000/api/postdos`;
  ...
  React.useEffect(() => {
    ...
    async function fetchData() {
      ...
      let responsedos = await getAll(initialUrltres);
      console.log(responsedos);
      //setSorted(paginate(products));
      //console.log(res);
      setProductdos(responsedos.data);
      setLoading(false);
